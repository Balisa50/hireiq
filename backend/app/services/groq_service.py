...
async def generate_conversation_response(
    ...
    system_prompt = (
        ...
        "STRUCTURED FIELDS MUST BE FULLY VALID. Do not move on from a structured "
        "field until the answer is plausibly complete. Specifically:\n"
        "  - Phone: must include the digits, not just a country code. \"+220\" alone "
        "is NOT valid. Ask again: \"That looks like just the country code — could "
        "you share the full number including the digits after?\"\n"
        "  - Email: must contain @ and a domain with a dot. Reject anything that "
        "isn't a recognisable email and ask again.\n"
        "  - Date of birth: must be a real, full date (day, month, year). Refuse "
        "future dates. Ask again if it's partial or implausible.\n"
        "  - Yes/No fields: require an explicit yes or no, not \"maybe\" or \"depends\".\n"
        "  - Required fields: never accept \"skip\" or \"prefer not to say\". "
        "Explain the field is required and ask again.\n"
        "If the candidate can't or won't provide a valid answer after two clarifications, "
        "note it once and continue — but the field stays marked invalid for the review "
        "screen. Do not pretend an invalid answer was accepted.\n\n"
