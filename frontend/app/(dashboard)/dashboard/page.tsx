...
import { Users, Plus, CheckCircle2, XCircle, Clock } from "lucide-react";
...
function EmptyActivity() {
  return (
    <div className="px-5 py-16 text-center">
      <svg className="w-14 h-14 mx-auto mb-5 text-border" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="1.5">
  ...
  useEffect(() => {
    ...
    function tryLoad() {
      ...
      companyAPI.getDashboardStats()
        .then(setStats)
        .catch(() => { if (attempts < 3) setTimeout(tryLoad, 1200 * attempts); })
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        {isFirstTime ? (
          <>
            <h1 className="text-xl font-semibold text-ink">Welcome to HireIQ, {company?.company_name}.</h1>
            <p className="text-sub text-sm mt-1">Your hiring dashboard is ready.</p>
            <Link href="/jobs/new" className="inline-flex items-center gap-1 text-[13px] text-sub hover:text-ink transition-colors mt-2">
      ...
      <div className="bg-white border border-border rounded-[4px] overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          ...
          <Link href="/candidates" className="text-[13px] text-sub hover:text-ink transition-colors">View all →</Link>
        </div>
        ...
        {isLoading ? (
          ...
          stats.recent_activity.map((a, i) => {
            ...
            return (
              <Link key={i} href="/candidates"
                className="px-5 py-3.5 flex items-center gap-4 border-b border-border last:border-b-0 hover:bg-[var(--bg)] transition-colors">
